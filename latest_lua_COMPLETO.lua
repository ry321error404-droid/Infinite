...
GoSpawnButton = Instance.new("TextButton")
DelSpawnButton = Instance.new("TextButton")
...
-- ===== SPAWNPOINT COMMANDS =====
addcmd('setspawnpoint',{'setspawn','spawnsave'},function(args, speaker)
	local char = speaker.Character
	if not char then
		notify('SetSpawnPoint','Você precisa de um personagem')
		return
	end
	
	local root = char:FindFirstChild("HumanoidRootPart")
	if not root then 
		notify('SetSpawnPoint','HumanoidRootPart não encontrado')
		return 
	end
	...
	_G.SpawnPointData = posData
	notify('SetSpawnPoint','Spawnpoint salvo em: ' .. math.floor(root.Position.X) .. ', ' .. math.floor(root.Position.Y) .. ', ' .. math.floor(root.Position.Z) .. ' ✓')
end)

addcmd('gotospawn',{'spawnpoint','spawn','gotosp'},function(args, speaker)
	if not _G.SpawnPointData then
		notify('GoToSpawn','Nenhum spawnpoint foi definido. Use setspawnpoint primeiro!')
		return
	end
	
	local char = speaker.Character
	if not char then 
		notify('GoToSpawn','Personagem não encontrado')
		return 
	end
	
	local root = char:FindFirstChild("HumanoidRootPart")
	if not root then 
		notify('GoToSpawn','HumanoidRootPart não encontrado')
		return 
	end
	
	-- Teleporta para o spawnpoint
	local posData = _G.SpawnPointData
	root.CFrame = CFrame.new(posData.X, posData.Y + 3, posData.Z)
	notify('GoToSpawn','Teleportado para o spawnpoint! ✓')
end)

addcmd('delspawnpoint',{'deletespawnpoint','rmspawn','delspawn'},function(args, speaker)
	_G.SpawnPointData = nil
	notify('DelSpawnPoint','Spawnpoint deletado ✓')
end)

addcmd('spawnpointinfo',{'spawninfo','spinfo'},function(args, speaker)
	if not _G.SpawnPointData then
		notify('SpawnInfo','Nenhum spawnpoint salvo')
		return
	end
	
	local data = _G.SpawnPointData
	notify('SpawnInfo','X: ' .. math.floor(data.X) .. ' | Y: ' .. math.floor(data.Y) .. ' | Z: ' .. math.floor(data.Z))
end)
-- ===== FIM SPAWNPOINT COMMANDS =====

-- ===== CONFIGURAÇÃO DOS BOTÕES DE SPAWN NA GUI =====
SpawnButton.Name = "SpawnButton"
SpawnButton.Parent = Holder
SpawnButton.BackgroundColor3 = Color3.fromRGB(76, 175, 80)
SpawnButton.BorderSizePixel = 0
SpawnButton.Position = UDim2.new(0, 5, 0, 200)
SpawnButton.Size = UDim2.new(0, 73, 0, 17)
SpawnButton.Font = Enum.Font.SourceSans
SpawnButton.TextSize = 12
SpawnButton.Text = "Set Spawn"
SpawnButton.TextColor3 = Color3.new(1, 1, 1)
SpawnButton.ZIndex = 10
table.insert(shade2, SpawnButton)

GoSpawnButton.Name = "GoSpawnButton"
GoSpawnButton.Parent = Holder
GoSpawnButton.BackgroundColor3 = Color3.fromRGB(33, 150, 243)
GoSpawnButton.BorderSizePixel = 0
GoSpawnButton.Position = UDim2.new(0, 83, 0, 200)
GoSpawnButton.Size = UDim2.new(0, 73, 0, 17)
GoSpawnButton.Font = Enum.Font.SourceSans
GoSpawnButton.TextSize = 12
GoSpawnButton.Text = "Go Spawn"
GoSpawnButton.TextColor3 = Color3.new(1, 1, 1)
GoSpawnButton.ZIndex = 10
table.insert(shade2, GoSpawnButton)

DelSpawnButton.Name = "DelSpawnButton"
DelSpawnButton.Parent = Holder
DelSpawnButton.BackgroundColor3 = Color3.fromRGB(244, 67, 54)
DelSpawnButton.BorderSizePixel = 0
DelSpawnButton.Position = UDim2.new(0, 161, 0, 200)
DelSpawnButton.Size = UDim2.new(0, 84, 0, 17)
DelSpawnButton.Font = Enum.Font.SourceSans
DelSpawnButton.TextSize = 12
DelSpawnButton.Text = "Delete Spawn"
DelSpawnButton.TextColor3 = Color3.new(1, 1, 1)
DelSpawnButton.ZIndex = 10
table.insert(shade2, DelSpawnButton)
...
SpawnButton.MouseButton1Click:Connect(function()
	if Players.LocalPlayer and Players.LocalPlayer.Character then
		...
		if root then
			...
			_G.SpawnPointData = posData
			notify('SetSpawnPoint','Spawnpoint salvo! ✓')
		end
	end
end)

GoSpawnButton.MouseButton1Click:Connect(function()
	if not _G.SpawnPointData then
		notify('GoToSpawn','Defina um spawnpoint primeiro!')
		return
	end
	...
	if Players.LocalPlayer and Players.LocalPlayer.Character then
		...
		if root then
			local posData = _G.SpawnPointData
			root.CFrame = CFrame.new(posData.X, posData.Y + 3, posData.Z)
			notify('GoToSpawn','Teleportado! ✓')
		end
	end
end)

DelSpawnButton.MouseButton1Click:Connect(function()
	_G.SpawnPointData = nil
	notify('DelSpawnPoint','Spawnpoint deletado ✓')
end)
-- ===== FIM BOTÕES DE SPAWN =====
